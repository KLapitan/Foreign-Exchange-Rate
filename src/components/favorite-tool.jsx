...
return(
      ...
      <div className={` h-auto  border-white p-2 ${favoriteContent ? "h-auto" : "h-40"}`}>
       ...
       {favoriteContent &&
        <>
          <h2 className="text-white  flex flex-row justify-between">FAVORITES <span>{favoriteList.length > 1 ? favoriteList.length + " pairs" : ""} </span></h2>
            <ul className="flex flex-col gap-3 h-auto mt-5 ">
        {favoriteList.map((item) => {

            const isfilled = favoriteList.some(fav => fav.to === item.to);

        return (
        <li className="flex flex-row text-white bg-BlackLight  items-center   border h-auto p-2  justify-between rounded-md">
        <span className="flex flex-col  sm:flex-row gap-2 sm:gap-20">
